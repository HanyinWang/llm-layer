...
if openai.api_key is None:
    from MedRAG.src.config import config
...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./MedRAG/corpus", cache_dir=None, condition='cancer'):
        ...
        self.templates = {"cot_system": general_cot_system, "cot_prompt": general_cot,
                    "medrag_system": general_medrag_system.replace('&&&', self.condition), "medrag_prompt": diagnostic_mistral_medrag}
        if self.llm_name.split('/')[0].lower() == "openai":
            ...
            if ("mixtral" in llm_name.lower()) or ("mistral" in llm_name.lower()):
                self.tokenizer.chat_template = open('./MedRAG/templates/mistral-instruct.jinja').read().replace('    ', '').replace('\n', '')
                self.max_length = 32768
                self.context_length = 30000
                ...
                self.tokenizer.chat_template = open('./MedRAG/templates/meditron.jinja').read().replace('    ', '').replace('\n', '')
                self.max_length = 4096
                self.context_length = 3072
                self.templates["cot_prompt"] = meditron_cot
                self.templates["medrag_prompt"] = meditron_medrag
            elif "pmc_llama" in llm_name.lower():
                self.tokenizer.chat_template = open('./MedRAG/templates/pmc_llama.jinja').read().replace('    ', '').replace('\n', '')
                self.max_length = 2048
                self.context_length = 1024
