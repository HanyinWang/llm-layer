...
if __name__ == "__main__":
    ...
    if not os.path.exists("MedRAG/corpus/wikipedia/chunk"):
        os.makedirs("MedRAG/corpus/wikipedia/chunk")
    ...
    for i in tqdm.tqdm(range(len(dat['train']))):
        ...
        if os.path.exists("MedRAG/corpus/wikipedia/chunk/wiki20220301en{:s}.jsonl".format(str(save_id).rjust(len_just, '0'))):
            continue
        ...
        if (i + 1) % batch_size == 0:
            with open("MedRAG/corpus/wikipedia/chunk/wiki20220301en{:s}.jsonl".format(str(save_id).rjust(len_just, '0')), 'w') as f:
                f.write('\n'.join(saved_text))
            saved_text = []
    if len(saved_text) > 0:
        with open("MedRAG/corpus/wikipedia/chunk/wiki20220301en{:s}.jsonl".format(str(save_id).rjust(len_just, '0')), 'w') as f:
